 ...
 \algoritmo{posAgente}{in as:as, in placa:agente}{res:pos}{
  ...
  \algoritmo{ingresarEstudiante}{in/out as:as, in nombre:string, in pos:pos}{}{
    ...
    \If {$as.atrapadoPorAgente?(pos)$}
      \State $as.sancionarAgentesVecinos(pos)$ \complejidad{O(1)}
    \EndIf
    \\
    \\
    %%%% TODO: Hippificarlo: Ver especificacion
    \State // Hippificar al estudiante
    \If {$as.estAHippie?(pos)$}
      \State $as.hippificar(pos)$ \complejidad{O(long(nombre))}
    \EndIf
    \\
    \\
    %%%% Convertir a los hippies que quedaron encerrados por 4 estudiantes o eliminar a los que quedaron atrapados por agentes
    %%%% Funciona bien porque los vecinos de una posicion no son vecinos entre ellos
    \State // Convertir a los hippies que quedaron encerrados por 4 estudiantes o eliminar a los que quedaron atrapados por agentes
    \State $as.aplicarHippiesVecinos(pos)$ \complejidad{O(long(nombre))}
    ...
    %%%%       Sino: agregar el estudiante al diccionario
...
\subsection{Algoritmos operaciones auxiliares}
 \algoritmo{sancionarAgentesVecinos}{in/out as:as, in pos:pos}{}{
    % Sancionar VECINOS
    \State $vecinos \larr as.campusEstatico.obtenerVecinos(pos)$ \complejidad{O(1)}
    \State $//$ La cantidad de vecinos es como maximo 4
    \If {$as.atrapadoPorAgente?(pos, vecinos)$} 
       \While {$i < vecinos.tamanio()$}  \complejidad{O(1)}
           \If {$as.campus[vecinos[i].x][vecinos[i].y].hayAgente?$}
               \State $as.sancionarAgente(vecinos[i].agente.siguiente)$ \complejidad{O(1)}
           \EndIf
           \State $i++$
       \EndWhile
    \EndIf
 ...
 \algoritmo{atrapadoPorAgente?}{in pos:pos, in vecinos:arreglo(pos)}{res:bool}{
    \State $alMenos1Agente \larr False$ \complejidad{O(1)}
    \State $i \larr 0$
    \While {$i<vecinos.tamanio()$} \complejidad{O(1)}
       \If {$as.campus[vecinos[i].x][vecinos[i].y].hayAgente?$}
          \State $alMenos1Agente$ $\larr$ $True$
          \State $i \larr vecinos.tamanio()$
        \EndIf

        \If {$(as.campus[vecinos[i].x][vecinos[i].y].hayAgente?$ $\lor$
             $as.campus[vecinos[i].x][vecinos[i].y].hayEst?$ $\lor$
             $as.campus[vecinos[i].x][vecinos[i].y].hayHippie?$ $\lor$
             $as.campus[vecinos[i].x][vecinos[i].y].hayHippie?)$}
              \State $return$ $false$ \complejidad{O(1)}
        \EndIf
        \State $i++$
    \EndWhile

    \State $res$ $\larr$ $alMenos1Agente$
 }{O(1)}